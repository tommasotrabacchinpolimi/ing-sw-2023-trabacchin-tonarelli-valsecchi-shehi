...
class BookShelfTest {
    ...
    void convertToTileTypeMatrix() {
        ...
        for(int i = 0; i < 6; i++){
            for(int j = 0; j < 5; j++){
                if (i==0 && j==0) assertNull(bookShelf.convertToTileTypeMatrix()[i][j]);
                else assertEquals(typesExpected[i][j].toString(), bookShelf.convertToTileTypeMatrix()[i][j].toString());
            }
        }
    }
}