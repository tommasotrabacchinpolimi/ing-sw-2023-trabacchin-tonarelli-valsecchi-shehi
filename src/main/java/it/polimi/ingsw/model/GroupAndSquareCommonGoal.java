...
public class GroupAndSquareCommonGoal extends CommonGoal implements Serializable {
    ...
    public List<EntryPatternGoal> rule(TileType[][] bookShelf) {
        ...
        for(TileType tileType : TileType.values()){
            ...
            List<Set<EntryPatternGoal>> sameTypeResult = getGroups(generateGroups(this.adjacentTilesPo2,entries).stream().filter(g->!square||isSquare(g,bookShelf.length,bookShelf[0].length).isPresent()).collect(Collectors.toList()), this.groupsNumber,this.separated);
