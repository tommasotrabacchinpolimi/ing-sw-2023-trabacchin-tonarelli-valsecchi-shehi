...
public class RmiAccepter<L extends RemoteInterface,R extends RemoteInterface> implements RemoteAccepterInterface<L,R>{
    ...
    public RmiAccepter(int portNumber, NetworkMonitor<R> networkMonitor, UserAccepter<R> userAccepter, L localTarget, TypeToken<R> remoteTargetClass, TypeToken<L> localTargetClass, ExecutorService executorService){
    ...
    public RmiAccepter(int portNumber, NetworkMonitor<R> networkMonitor, UserAccepter<R> userAccepter, Object localTarget, TypeToken<R> remoteTargetClass, TypeToken<L> localTargetClass,ExecutorService executorService,Supplier<UserAdapterInterface<R>> userAdapterSupplier){
        this.localTarget = localTarget;
        this.userAccepter = userAccepter;
        this.executorService = executorService;
        this.networkMonitor = networkMonitor;
    ...
    public L register(R remoteObject) throws RemoteException {
        ...
        if(userAccepter.acceptUser(user)){
            ...
            if(userAdapterSupplier==null){
                rmiConnectionManager.init(portNumber,networkMonitor,user,remoteTargetClass,localTargetClass,(L)localTarget,remoteObject,executorService);
            }
            else{
                ...
                rmiConnectionManager.init(portNumber,networkMonitor,user,remoteTargetClass,localTargetClass,newLocalTarget,remoteObject,executorService);
